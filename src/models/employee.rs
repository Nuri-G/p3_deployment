...
/// Representation of Employees in the database.
...
/// Inserts the posted employee into the database. If id is None, it will be generated by the database.
#[post("/api/employees")]
...
/// Uses the Employee extractor to get it from the values in the JWT in the header.
/// Returns the employee as a JSON object.
#[post("/api/auth")]
pub async fn user_from_token(e: Employee) -> Result<impl Responder> {
    Ok(web::Json(e))
}

/// Custom error type needed to implement FromRequest for Employee.
#[derive(Debug)]
...
/// Converts a JWT in the request header into an Employee.
